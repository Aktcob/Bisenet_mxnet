...
class LRScheduler(lr_scheduler.LRScheduler):
    ...
    def update(self, i, epoch, epochs):
        T = epoch * self.niters + i
        max_iters = epochs * self.niters
        assert(0 <= T <= self.N)
        ...
        if self.warmup_epochs > epoch:
            ...
            if self.mode == 'step':
                ...
                # print (pow(1 - T/max_iters, self.power))
                # self.learning_rate = self.baselr * pow(1 - T/max_iters, self.power)
